...
def register_handlers(bot: TeleBot):
    ...
    def update_activity(message: types.Message):
        ...
        try:
            user = db.get_user(telegram_id)
            activities = db.get_recent_activities(user[0])  # Get recent activities
            ...
            if activities:
                ...
                for activity in activities:
                    ...
                    keyboard.add(f"{activity_id}: {activity_name} ({activity_type})")
                bot.reply_to(message, "Please choose an activity to update:", reply_markup=keyboard)
    ...
    def process_update_activity_choice(message: types.Message, activities):
        ...
        try:
            ...
            activity = next((act for act in activities if act[0] == activity_id), None)
            
            if not activity:
                raise ValueError("Invalid activity selection")
            
            activity_id, activity_name, current_value, activity_type, created_at = activity
            
            if activity_type == 'time':
                bot.reply_to(message, f"Updating activity: {activity_name} (ID: {activity_id})\nCurrent type: {activity_type}\nPlease enter the new duration in the format HH:MM:SS:", reply_markup=types.ReplyKeyboardRemove())
            else:
                bot.reply_to(message, f"Updating activity: {activity_name} (ID: {activity_id})\nCurrent type: {activity_type}\nPlease enter the new number of reps:", reply_markup=types.ReplyKeyboardRemove())
    ...
    def process_update_activity_value(message: types.Message, activity_id, activity_type):
        if check_maintenance(message, bot):
            return
        if check_exit(message, bot):
            return
        ...
        try:
            ...
            if activities:
                ...
                for activity in activities:
                    ...
                    keyboard.add(f"{activity_id}: {activity_name} ({activity_type})")
                bot.reply_to(message, "Please choose a reference activity to update:", reply_markup=keyboard)
    ...
    def process_update_reference_activity_choice(message: Message, activities):
        ...
        try:
            ...
            bot.reply_to(message, f"Updating reference activity: {current_name} (ID: {activity_id})\nCurrent type: {current_type}\nPlease enter the new name (or 'skip' to keep the current name):", reply_markup=types.ReplyKeyboardRemove())
    ...
    def process_delete_reference_activity_check(message: Message):
        ...
        try:
            ...
            if activity_count > 0:
                ...
                keyboard.add("Yes", "No")
                bot.reply_to(message, 
                             f"Warning: The reference activity '{reference_activity[0]}' has {activity_count} recorded activities. "
                             f"Deleting this reference will also delete all associated activities. "
                             f"Are you sure you want to proceed?",
                             reply_markup=keyboard)
                bot.register_next_step_handler(message, process_delete_reference_activity_confirm, activity_id)
